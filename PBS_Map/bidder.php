<?php
	...
	$RTB 	= array();
	$config = array();
	$apn_ver = ( $_GET['v'] == 'dr' || $_GET['v'] == 'pbs') ? $_GET['v'] : 'dr';
	$apn_placement = ( is_numeric($_GET['pid']) ) ? $_GET['pid'] : '16077465';
	
	$config['pbs_pbs']	= 'http://ib.adnxs.com/openrtb2';
	$config['pbs_dr']	= 'http://yieldnexus-useast.adnxs.com/openrtb2';
	$config['pbs_port']	= 80;
	$config['apnxs_id']	= '1db9eb10-7c39-48ac-aad6-cc96184f1c8c';
	$config['bidders']	= '
							{
								"appnexus": {
									"placement_id": ' . $apn_placement . '
								}
							}
						';
	...
	$ch = curl_init( $config['pbs_' . $apn_ver ] );
	...
	switch ( $bid_status ) {
		case 204:
			...
			log_message("HTTP/1.0 204 : " . json_encode( $RTB ), 0, 1);
			break;
		case 404:
		...
		case 200:
			...
			if ( $bid_response_arr['seatbid'] ) {
				print( $bid_response );
				log_message("SUCCESS for : " . json_encode( $RTB ) . " :: " . $bid_response, 0, 10);
			} else {
				header("HTTP/1.0 204 EMPTY");
				log_message("HTTP/1.0 204 B: " . json_encode( $RTB ), 0, 1);
			}
			break;
		default:
			header("HTTP/1.0 " . $bid_status );
			log_message("HTTP/1.0 " . $bid_status . ": " . json_encode( $RTB ), 1, 10);
	}
	...
	function log_message( $message, $error = 0, $rate = 10 ) {
		...
		if ( $p < $rate || 1==1 ) {
			if ( $error ) {
				error_log( $message . "\n", 3, "/var/log/bidder_".$apn_ver."_error_log.log");
			} else {
				error_log( $message . "\n", 3, "/var/log/bidder_".$apn_ver."_message_log.log");
			}
		}
	}
